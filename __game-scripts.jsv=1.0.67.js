...
Attachment.prototype.setMuzzle = function() {
    ...
    this.currentSparkAngle = 90 * Math.random(),
    this.muzzleSmoke.particlesystem.reset(),
    this.muzzleSmoke.particlesystem.play(),
    this.muzzleSpark.enabled = !0,
    this.renderFire = 2;
...
Attachment.prototype.updateBullets = function() {
    for (var t = this.batchBullets.length; t--; )
        this.batchBullets[t].rotateLocal(6, 0, 0),
        this.batchBullets[t].translateLocal(-.055, this.batchBullets[t].direction, .01),
...
Attachment.prototype.update = function(t) {
    this.renderFire < 0 && (this.muzzleSpark.enabled = !1),
...
Movement.prototype.initialize = function() {
    ...
    this.heightVector = new pc.Vec3(0,-5,0),
...
Movement.prototype.mouseDown = function(t) {
    ...
    if (this.locked)
        return !1;
    if (this.interface.isInventoryEnabled || this.app.mouse.enablePointerLock(),
    this.onLeftHand && this.onTablet)
        return !1;
    if (!pc.Mouse.isPointerLocked())
        return !1;
    (2 == t.button && (this.currentWeaponDetails.noFocus || (this.isFocusing = !0,
    this.entity.sound.play("Focus"))),
    0 === t.button && this.isReloading < 0 && this.isVaccinate < 0) && (this.isShooting = !0,
    this.shoot(),
...
Movement.prototype.setRaycast = function() {
    var t = this.app.graphicsDevice.width / 2
      , e = this.app.graphicsDevice.height / 2;
...
Movement.prototype.doRaycast = function(t) {
    if (!t && Date.now() - this.lastRaycast < 100)
        return !1;
    var e = !1
      , i = this.app.graphicsDevice.width / 2
      , s = this.app.graphicsDevice.height / 2;
    for (var n in this.raycast.from = this.cameraEntity.camera.screenToWorld(i, s, this.cameraEntity.camera.nearClip),
    this.setRaycast(),
    this.ray.origin.copy(this.raycast.from),
    this.ray.direction.copy(this.raycast.to).normalize(),
    this.updateInteractiveObjects(),
    this.interactiveObjects) {
        var o = this.interactiveObjects[n];
        if (o && o.enabled && o.script && o.script.interactive)
            o.script.interactive.aabbShape.intersectsRay(this.ray, this.hitPosition) && (this.setInteractive(o, o.script.interactive),
            e = !0)
    }
...
Movement.prototype.setInteractive = function(t, e) {
    e.getDetails().isAvailable && (this.inspecting = t,
    this.outlineShaderEntity.script.outline.activeEntity = t.children[0],
    this.interface.setInspect(e.getDetails()))
}
,
...
Movement.prototype.mouseMove = function(t) {
    if (!this.isMovementAllowed())
        return !1;
    if ((pc.Mouse.isPointerLocked() || t.buttons[0]) && !this.inControl && !this.locked) {
...
Movement.prototype.setMovement = function() {
    ...
    this.interface.showFeeling("breathing", 6))),
...
Movement.prototype.useNeedle = function() {
    return !(this.isTaking < 0 && this.isReloading < 0 && this.isSwinging < 0 && this.isThrowingStarted) && (this.inventory.hasItem("Needle") ? (this.disableTablet(),
    this.onLeftHand = !0,
...
Movement.prototype.throwGrenade = function() {
    this.isTaking < 0 && this.isReloading < 0 && this.isVaccinate < 0 && this.isSwinging < 0 && this.isThrowingStarted && (this.isThrowing = 50,
    this.isThrowingStarted = !1,
...
Movement.prototype.setWeapon = function(t, e) {
    if (this.isReloading > 0 || this.isVaccinate > 0 || this.isTaking > 0 || this.isSwinging > 0)
        return !1;
    if (t)
...
Movement.prototype.takeout = function() {
    this.isTaking < 0 && (0 === this.currentWeaponDetails.currentAmmo ? this.reload(!0) : (this.isTaking = 200,
...
Movement.prototype.setHeight = function() {
    if (this.currentDate - this.lastHeightRaycast < 100)
        return !1;
    var t = this.entity.getPosition().clone()
...
Movement.prototype.applyFallForce = function() {
    ...
    this.jumpVector.z = 0,
    this.jumpForwardVector.x = this.cameraEntity.forward.x,
    this.jumpForwardVector.y = 0,
    this.jumpForwardVector.z = this.cameraEntity.forward.z,
    this.jumpVector.add(this.jumpForwardVector.scale(.2)),
...
Movement.prototype.reload = function(t) {
    if (this.isReloading < 0 && this.isVaccinate < 0 && this.isTaking < 0 && this.isSwinging < 0 && this.currentWeaponDetails) {
        ...
        if (t && 0 !== e)
            return !1;
        this.currentWeaponDetails.clipCapacity == this.currentWeaponDetails.currentAmmo || (e > 0 ? (this.isFocusing = !1,
        this.isReloading = 125,
        this.currentWeaponDetails.isShotgun ? (this.entity.sound.play("Shotgun-Open"),
        ...
        this.useAmmo(this.currentWeaponDetails.clipCapacity, this.currentWeaponDetails.currentAmmo, e)),
        this.inventory.updateLeftSpace()) : this.interface.showAlert("You don't have enough " + Utils.clearName(this.currentWeaponDetails.ammoType) + " ammo!", "default", this.currentWeaponDetails.ammoType))
    }
}
,
Movement.prototype.updateAnimation = function() {
    var t = 0
      , e = 0
      , i = 0;
    if (!this.isShooting && !this.isThrowingStarted && this.isReloading < 0 && this.isVaccinate < 0 && this.isTaking < 0 && this.isSwinging < 0 && this.isThrowing < 0 && !this.locked) {
        if (this.currentSpeed >= 1 && this.shootRate < 0 && !this.isFalling) {
    ...
    this.animationCallback && this.armsEntity.animation.currentTime > 0 && this.armsEntity.animation.currentTime >= this.armsEntity.animation.duration - .05 && (this.animationCallback(this),
    this.animationCallback = !1)
}
,
...
Movement.prototype.setCameraAngle = function(t) {
    ...
    this.currentJumpingForce > 0 && (this.currentEuler.y = pc.math.lerp(this.currentEuler.y, this.currentJumpingForce, .4)),
    this.isJumping > 0 ? this.isJumping > this.maxJumpTime / 2 ? this.armsX = pc.math.lerpAngle(this.armsX, 10, .1) : this.isJumping < 5 ? this.armsX = pc.math.lerpAngle(this.armsX, 1, .1) : this.armsX = pc.math.lerpAngle(this.armsX, -5, .1) : this.armsX = pc.math.lerpAngle(this.armsX, 0, .1),
    ...
    var i = this.cameraPoint.getLocalEulerAngles();
    this.isDeath && !this.inControl || (this.pitchEntity.setLocalEulerAngles(this.currentPitchAngle.y, 0, 0),
    ...
    this.cameraEntity.setLocalEulerAngles(.5 * this.currentBounceX + .1 * this.armsX + this.senseY + i.y + this.quickForceX + this.runVibration, 0, .5 * this.currentBounceY - this.senseX + i.x)),
    ...
    this.handPivotEntity.setLocalEulerAngles(.15 * this.swingX, .15 * this.swingY, 0)
}
,
...
Movement.prototype.fireBullet = function() {
    ...
    e = Math.floor(e),
    this.currentWeaponDetails.bigScope && !this.isFocusing ? (t.x += 300 * Math.random() - 300 * Math.random(),
    t.y += 300 * Math.random() - 300 * Math.random(),
    t.z += 300 * Math.random() - 300 * Math.random()) : this.currentWeaponDetails.isShotgun ? (t.x += 60 * Math.random() - 60 * Math.random(),
    t.y += 60 * Math.random() - 60 * Math.random(),
    t.z += 60 * Math.random() - 60 * Math.random()) : this.isFocusing || (t.x += 1 * this.currentRecoilForce * Math.random() - 1 * this.currentRecoilForce * Math.random(),
    t.y += 1 * this.currentRecoilForce * Math.random() - 1 * this.currentRecoilForce * Math.random(),
    t.z += 1 * this.currentRecoilForce * Math.random() - 1 * this.currentRecoilForce * Math.random()),
...
Movement.prototype.shoot = function() {
    ...
    if (this.isShooting && this.nextShootTime < 0) {
        var t = this.currentWeaponDetails
          , e = t.recoil;
        e *= 1.2,
        !1 === this.isFocusing ? t.automatic && "Handgun" == t.type ? this.setAnimation("Idle", 1, 0, !1, null, !0) : this.setAnimation("Fire", 1, 0, !1, null, !0) : e *= .3,
        ...
        this.nextBodyRotation.x -= 15 * e * Math.random(),
        this.nextBodyRotation.y -= 15 * e * Math.random(),
        this.nextBodyRotation.z -= 15 * e * Math.random(),
        this.currentRecoilForce += 15 * e,
        this.fireBullet(),
...
Movement.prototype.setFire = function() {
    this.shootRate < 0 && !this.shootLock && this.isThrowing < 0 && this.shoot()
}
,
Movement.prototype.setBodyPosition = function() {
    ...
    this.shootRate > 0 && (i.y += .001 * Math.sin(this.timestamp / 4),
...
Movement.prototype.showResults = function(t) {
    ...
    setTimeout(function() {
        e.locked = !0,
        e.isForceStop = !0,
        e.giveControl(e.killerEntity),
        e.setAnimation("Idle", 1, e.blendTime, !0),
...
,
Movement.prototype.postUpdate = function(t) {
    this.setCurrentDate(),
    this.setExposure(),
    this.checkHealth(),
...
Interface.prototype.setCaseHealthPosition = function() {
    ...
    if (!1 !== this.caseEntity && this.caseEntity.enabled) {
        ...
        e.x > 0 && e.x < n.width && e.y > 0 && e.y < n.height && a > 0 && s ? (this.caseHealthBar.setLocalScale(a, 1, 1),
...
Interface.prototype.addHitPoint = function(t, e) {
    var i = pc.math.random(-10, 10)
      , n = this.hitPoint.clone();
    n.enabled = !0,
    n.setLocalPosition(i, i, 0),
    n.element.text = Math.floor(t) + "",
    n.tween(n.getLocalPosition()).to({
        x: 0,
        y: 400,
        z: 0
    }, 1.5, pc.CircularOut).start(),
    n.tween(n.element).to({
        opacity: 0
    }, 1, pc.CircularOut).start(),
    this.hitPointsWrapper.addChild(n),
...
Interface.prototype.setGuidePosition = function() {
    ...
    if (!1 !== this.lastGuidePosition) {
        ...
        var i = this.guideEntity.element.screen.screen.scale
          , n = this.app.graphicsDevice;
        e.x > 0 && e.x < n.width && e.y > 0 && e.y < n.height ? this.guideEntity.setLocalPosition(e.x / i, (n.height - e.y) / i, 0) : this.hideGuide()
    }
}
,
...
Interface.prototype.update = function(t) {
    ...
    this.reloadingBar.setLocalScale(100 * this.character.isReloading / 125 * .01, 1, 1)) : this.reloadingElement.enabled = !1,
    ...
    a) {
        if (a.entity.tags.list().indexOf("State") > -1 && !a.entity.state)
            ...
            if (o > 0) {
                ...
                a.entity.script.enemy.damage(t, o, a.normal, h)
            }
...
,
Enemy.prototype.setCounters = function() {
    this.lastShootTime--,
    this.isJumping--,
    this.isThrowing--
}
...
ItemSpawn.prototype.spawnItem = function(t, e) {
    ...
    !!(i = this.items[this.index]) && this.dropItem(i.name, t.getPosition().clone(), -1, !0)
}
,
...
,
ItemSpawn.prototype.dropItem = function(t, e, i, s) {
    var n = new pc.Vec3(0,90,0)
      , a = this.findItem(t).clone();
    return e.add(new pc.Vec3(0,.05,0)),
    a.setPosition(e),
    a.setLocalEulerAngles(n),
    a.type = a.script.weapon.type,
    a.enabled = !0,
    s || a.script.weapon.setHeight(),
    i > -1 && (a.script.weapon.amount = i),
    this.dynamicWorld.addChild(a),
    a
}
,
...
Menu.prototype.openReward = function(t) {
    ...
    "skin" == t.type && (this.rewardTitle.enabled = !0,
...
Menu.prototype.setScreenSize = function() {
    if (Date.now() - this.lastScreenSizeUpdate < 100)
        return !1;
    for (var t = this.entity.screen.scale, e = this.entity.screen._resolution, n = this.screenSizeElements.length; n--; ) {
        var i = this.screenSizeElements[n];
        i.enabled && (i.element.calculatedWidth = e.x / t,
        i.element.calculatedHeight = e.y / t)
    }
    this.lastScreenSizeUpdate = Date.now()
}
,
...
Menu.prototype.findMatch = function() {
    ...
    var e = "1.0.0";
    "undefined" != typeof VERSION && (e = VERSION),
    Service.post("find_match", {
        ...
        e.success ? e.session ? (pc.selectedMap = e.session.map,
        pc.network.IP = e.session.ip,
        pc.network.roomId = e.session.room_id,
        pc.network.radiusId = e.session.radius,
        t.loadScene("Game")) : alert("Couldn't find match, try again please") : (alert(e.message),
        t.playButton.script.button.resolve())
    })
}
,
...
Menu.prototype.loadScene = function(t) {
    var e = this
      , n = pc.app._sceneRegistry.find(t).url
      , i = pc.app.root.findByName("Root");
    "undefined" != typeof VERSION && (n = n + "?v=" + VERSION),
    pc.app.loadSceneHierarchy(n, function(t, n) {
...
Profile.prototype.getDetails = function() {
    ...
    Service.post("get_details", {
        ...
        if (t && t.success) {
            e.isAuth = !0,
            t.hash && (pc.hash = t.hash,
            pc.username = t.username,
            Service.set("hash", pc.hash)),
            e.usernameElement.element.text = t.username,
            e.levelElement.element.text = t.level + "",
            e.nextLevelElement.element.text = "Next level : " + (t.level + 1);
            Math.floor(t.experience);
            var i = Math.pow(10 * t.level, 2)
              , s = Math.pow(10 * (t.level - 1), 2)
              , n = t.experience - s
              , r = i - s;
            for (var o in e.experienceBar.element.width = 100 * n / r * 4,
            e.experienceElement.element.text = t.experience + " / " + i,
            100 * n / r > 10 ? (e.experienceElement.enabled = !0,
            e.shortcutExperienceElement.enabled = !0) : (e.experienceElement.enabled = !1,
            e.shortcutExperienceElement.enabled = !1),
            e.goldElements) {
                e.goldElements[o].element.text = t.coins + ""
            }
            e.killsElement.element.text = t.kills + "",
            e.deathsElement.element.text = t.deaths + "",
            e.headshotsElement.element.text = t.headshots + "",
            e.gameTimeElement.element.text = Utils.mmss(t.total_game_time) + "",
            e.lastMatchKillsElement.element.text = t.last_match_kills + "",
            e.lastMatchTimeElement.element.text = Utils.mmss(t.last_match_time) + "",
            e.lastMatchPositionElement.element.text = t.last_match_position + "",
            e.shortcutUsername.element.text = t.username,
            e.shortcutLevel.element.text = t.level + "",
            e.shortcutExperienceBar.element.width = 100 * n / r * 1.5,
            e.shortcutExperienceElement.element.text = t.experience + " / " + i,
            e.shortcutLoginButton.enabled = !1,
            e.shortcutExperienceHolder.enabled = !0,
            e.shortcutLevelBadge.enabled = !0,
            e.setProfilePicture(t.active_skin),
            e.getSkins(),
            e.setDefaultSkin(),
            e.setBadges(t.badges),
            e.disableLoginRequiredEntities(),
            setTimeout(function() {
                e.getFriends()
            }, 1e3),
            void 0 !== pc.miniplay && (pc.miniplay.save("total_kills", t.kills),
            pc.miniplay.save("total_deaths", t.deaths),
            pc.miniplay.save("total_experience", t.experience))
        } else
            e.isAuth = !1,
...
Profile.prototype.setBadges = function(e) {
    ...
    for (var i in t) {
        var s = t[i];
        if (s) {
            var n = this.app.assets.find(s + "-badge.png");
            if (n) {
                var r = this.badgeElement.clone();
                r.element.textureAsset = n.id,
                r.enabled = !0,
...
Profile.prototype.setFriends = function(e) {
    ...
    null != Service.get("followers") && Service.get("followers") < t.length) {
        var s = t.length - parseInt(Service.get("followers"));
        this.app.fire("Interface:Notification", 'You have [color="#FFFF00"]' + s + " new[/color] followers.")
    }
    for (var n in Service.set("followers", t.length),
    i) {
        var r = i[n];
        ...
        var o = this.friendRowEntity.clone();
        o.setLocalPosition(0, -parseInt(33 * n), 0),
        o.enabled = !0,
        this.friendsContent.addChild(o),
        this.followedUsers.push(o)
    }
    for (var l in t) {
        var a = t[l];
        this.followersRowEntity.findByName("Username").element.text = a.username,
        this.followersRowEntity.findByName("Follow").script.button.triggerFunction = "profile.followFriend('" + a.username + "');",
        a.active_skin ? this.followersRowEntity.findByName("ProfileImage").element.textureAsset = this.app.assets.find(a.active_skin + "-CharacterThumbnail") : this.followersRowEntity.findByName("ProfileImage").element.textureAsset = this.app.assets.find("no_photo.jpg");
        var d = this.followersRowEntity.clone();
        d.setLocalPosition(0, -parseInt(33 * l), 0),
        d.enabled = !0,
...
Profile.prototype.setSkins = function(e) {
    ...
    for (var n in s) {
        var r = s[n].name
          , o = this.app.assets.find(r + "-CharacterThumbnail");
        if (this.app.assets.find(r + "-Character") && -1 === t.indexOf(r)) {
            var l = parseInt(i) % 4
              , a = Math.floor(parseInt(i) / 4)
              , d = this.skinItem.clone();
            d.name = r,
            d.enabled = !0,
            d.activeIcon = d.findByName("Active"),
            d.activeIcon.enabled = !1,
            d.findByName("Name").element.text = r,
            d.findByName("Thumbnail").element.textureAsset = o,
            d.findByName("SwitchButton").script.button.triggerFunction = "menu.switchSkin('" + r + "');",
            d.setLocalPosition(160 * l, -160 * a, 0),
...
Parachute.prototype.initialize = function() {
    ...
    this.currentAngle = Math.floor(170 * Math.random()),
...
Parachute.prototype.cutParachute = function() {
    this.onGround || (this.entity.sound.stop("ParachuteFall"),
...
Parachute.prototype.setCurrentValues = function() {
    ...
    e = 50,
    a = 0),
    this.height < 3 && this.cutParachute(),
    this.onGround && (e = 60),
...
Parachute.prototype.applyGravityForce = function(t) {
    ...
    this.app.keyboard.isPressed(pc.KEY_W) && !this.autoControl && (this.accelerateSpeed = pc.math.lerp(this.accelerateSpeed, 3, .1),
    this.characterRotationX = pc.math.lerp(this.characterRotationX, 85, .01)),
    this.app.keyboard.isPressed(pc.KEY_A) && !this.autoControl && (this.currentAngle += this.turnSpeed,
    this.characterRotationZ = pc.math.lerp(this.characterRotationZ, -85, .01)),
    this.app.keyboard.isPressed(pc.KEY_D) && !this.autoControl && (this.currentAngle -= this.turnSpeed,
    this.characterRotationZ = pc.math.lerp(this.characterRotationZ, 85, .01)),
...
Parachute.prototype.update = function(t) {
    ...
    this.setAnimation(),
    this.setNetworkPosition(),
    this.onGround && this.focusTimeout++,
...
Translate.prototype.update = function(t) {
    "x" == this.axis && this.entity.translateLocal(this.speed, 0, 0),
    "y" == this.axis && this.entity.translateLocal(0, this.speed, 0),
    "z" == this.axis && this.entity.translateLocal(0, 0, this.speed)
}
;
...
Network.prototype.setTickStates = function() {
    if (this.time == this.radiusTime && this.app.fire("Event:Circle", this.radiusId),
    this.time == this.evacuateTime && this.controls && this.controls.interface && this.controls.interface.showEvacuateMessage(),
    this.time >= this.evacuateTime && this.controls && this.controls.interface) {
...
Container.prototype.updateStyle = function() {
    if (this.entity.element.screenCorners) {
        ...
        var e = (t[2].x - t[0].x) / this.entity.element.width
          , n = (t[2].y - t[0].y) / this.entity.element.height;
        this.autoResize && (this.element.style.transform = "scale(" + e + ", " + n + ")",
...
PhysicsManager.prototype.checkRigidbodies = function() {
    if (Date.now() - this.lastRigidUpdate < 300)
        return !1;
    if (!this.rigidbodies)
        return !1;
    ...
    this.lastRigidUpdate = Date.now(),
    Date.now() - this.lastDistanceUpdate >= 500 && (this.lastDistanceUpdate = Date.now())
}
,
PhysicsManager.prototype.update = function(i) {
    this.checkRigidbodies()
}
;
var Service = {
    baseURL: "https://gateway.miniroyale2.io",
    post: function(e, t, n) {
        var o = Service.baseURL + "?request=" + e
...
Leaderboard.prototype.initialize = function() {
    ...
    Service.post("leaderboard", {
        ...
        e.success ? t.updateRows(e.entries) : alert("An error occured!")
    }),
    this.rowEntity.enabled = !1
}
,
Leaderboard.prototype.updateRows = function(t) {
    for (var e = this.contentEntity.children.length; e--; )
        this.contentEntity.children[e].enabled && this.contentEntity.children[e].destroy();
    for (var i in t) {
        var n = t[i];
        this.rowEntity.findByName("Username").element.text = n.username,
        ...
        var r = this.rowEntity.clone();
        r.enabled = !0,
        r.setLocalPosition(0, -33 * i, 0),
        this.contentEntity.addChild(r)
    }
}
;
...
Ping.prototype.getCountry = function() {
    var t = this;
    Service.post("get_country", {}, function(n) {
        n && n.country && t.setCountry(n.country)
    })
}
,
Ping.prototype.testPing = function() {
    var t = this
      , n = Date.now();
    Service.post("ping", {}, function() {
        var e = (Date.now() - n) / 1e3
          , i = 1;
        e < 100 ? i = 3 : e < 300 ? i = 2 : e >= 300 && (i = 1),
        t.setSignalLevel(i)
    })
}
;
...
,
Helicopter.prototype.addUserMessage = function(t, e) {
    if (!t || !e)
        return !1;
    var i = this.notificationUserMessage.clone();
    i.username = i.findByName("Username"),
...
,
Helicopter.prototype.setJoinedPlayer = function(t, e, i, n) {
...
Helicopter.prototype.setEnemyDeployPosition = function(t, e, i, n) {
    if (t == e) {
        ...
        var o = (i + 1) / 350 * 80
          , s = (n + 1) / 350 * 80;
        this.parachutePlayerEntity.setLocalPosition(o, 100, s)
    } else {
...
LimitFps.prototype.limit = function(t) {
    ...
    t >= 60 ? i.autoRender = !0 : (i.autoRender = !1,
...
Ambient.prototype.initialize = function() {
    void 0 !== pc.selectedMap ? this.selectedMap = pc.selectedMap : this.selectedMap = "port",
    ...
    if (this.mapNames.length > 0)
        for (var s = this.mapNames.length; s--; )
            this.mapNames[s].element.text = this.maps[this.currentIndex];
